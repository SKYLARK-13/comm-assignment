...
function Team(props) {
  ...
  function handleAddMembers(key) {
    ...
    if (empName && (empPosition || empPosition == 0)) {
      ...
      setMember(
        <div className={classes.members_added}>
          <TeamMember team={key} department={props.department} />
        </div>
      );
      setShowMembersKey(key);
  ...
  return Object.keys(teams).map(function (key, item) {
    return (
        ...
        <Grid container spacing={3} className={classes.root}>
          ...
          {teams[`${key}`].members ? (
            <div>
              <button
                style={buttonStyle}
                onClick={() => handleShowMembers(key)}
              >
                Show Team
              </button>
              <button style={buttonStyle} onClick={() => handleAddMembers(key)}>
                Add Member
              </button>
            </div>
          ) : (
