...
import user from "../../constants/images/user.jpg";
...
function TeamMember(props) {
  ...
  const [data, setData] = useState(
    JSON.parse(localStorage.getItem("empsData"))
  );
  const [anchorEl, setAnchorEl] = useState(null);
  const [elKey, setElKey] = useState();
  var teams = data.ceo.departments[props.department].teams;
  var employees = data.ceo.departments[props.department].teams[
    props.team
  ].members.sort(GetSortOrder("position"));
  ...
  function handleTeamChange(event) {
    data.ceo.departments[props.department].teams[
      event.target.value
    ].members.push(employees.splice(elKey, 1)[0]);
    localStorage.setItem("empsData", JSON.stringify(data));
  }

  function handleDelete(key) {
    data.ceo.departments[props.department].teams[props.team].members.splice(
      key,
      1
    );
    localStorage.setItem("empsData", JSON.stringify(data));
  ...
  var members = Object.keys(employees).map((item, key) => (
    <Grid item xs={12} key={key}>
      <div className={classes.text_color}>
        <Avatar
          className={classes.avatar_size}
          alt={employees[`${key}`].emp_name}
          src={user}
        />
        <Popover
          id={id}
        ...
        <p>
          {employees[`${key}`].position == 0
            ? "Team Lead"
            : employees[`${key}`].position == 1
            ? "Level 1"
            : employees[`${key}`].position == 2
            ? "Level 2"
            : "Level 3"}
        </p>
      </div>
